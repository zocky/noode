...
var CLIENT = (function() {
  ...
  function getRowCount($result) {
    ...
    request('sql_querycount',{sql:sql}, function(data) {
      ...
      $result.toggleClass('paged',opt.totalrows > opt.limit);
      $result.find('.pagesize').each( function () {
        var $this = $(this);
        $this.css('display', $this.attr('data-pagesize') <= opt.totalrows ? 'inline-block' : 'none');
      });
    }, function() {
  ...
  function getGridPage($result,r) {
    ...
    $result.find('.filter').each(function() {
      var $this = $(this);
      opt.filters[$this.attr('data-column')]=$this.find(':input').val();
    })
    ...
    resQueryPage($result, sql,[],opt, function(data) {
      ...
      $result.find('.filter').each(function() {
        var $this = $(this);
        $this.find(':input').val(opt.filters[$this.attr('data-column')]);
      })
    });
  }
  ...
  function addSqlQuery(sql,opt) {
    ...
    resQuery($result, sql, function(data) {
      showSuccess('<b>'+ (data.command||'SQL') + ' OK:</b> ' + (data.rowCount|00)+' rows affected.',$result);
      $result.find('.placeholder').html( template(data.rows.length ? 'tpl_grid' : 'tpl_norows',{
        rows: data.rows
      }));
...
*/        
  ...
  $(function() {
    ...
    $('.result .button.reload').live('click',function() {
	  ...
	  $('.result .filter').live('change', function() {
      var $res = $(this).closest('.result');
      getRowCount($res);
      getGridPage($res);
	  })
	  $('#sql, .result .code').live('input', function() {
